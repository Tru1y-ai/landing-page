...
| Re-use | `.reveal-record` is `EvidenceChip`'s trace and threads, already built and debugged |
...
1. **Build Concept B as a scroll reveal** (§4d). A résumé that turns translucent as you
   ...
   re-uses `EvidenceChip`'s trace and threads, and the visitor performs the reveal
   themselves. One transformation, one variable. This is the hero.
