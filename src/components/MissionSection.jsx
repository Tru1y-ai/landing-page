  ...
  return (
    <section id="mission" className="border-t border-black/8" style={{ padding: '7rem 0' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '0 2.5rem' }} className="flex flex-col items-center text-center">
        ...
        />
        <SplitText
          text="AI is revolutionizing how work gets done. We're building the infrastructure to evaluate talent the right way — thoughtful, fair, and built for the modern workforce."
          tag="p"
      ...
      <div className="flex flex-col items-center gap-1" style={{ margin: '5rem 0' }}>
        <FuzzyText fontSize="1.1rem" fontWeight={500} color="#000" baseIntensity={0.08} hoverIntensity={0.35} enableHover>
          No more
        </FuzzyText>
        <FuzzyText fontSize="clamp(2.8rem, 5vw, 4.5rem)" fontWeight={900} color="#000" baseIntensity={0.12} hoverIntensity={0.45} enableHover glitchMode glitchInterval={3500} glitchDuration={180}>
          Stupid
        </FuzzyText>
        <FuzzyText fontSize="clamp(1.5rem, 2.5vw, 2.2rem)" fontWeight={800} color="#000" baseIntensity={0.1} hoverIntensity={0.4} enableHover>
          Technical Tests.
        </FuzzyText>
        <div style={{ marginTop: '1.5rem' }}>
          <FuzzyText fontSize="1rem" fontWeight={500} color="#00000088" baseIntensity={0.06} hoverIntensity={0.3} enableHover>
            It is time to show
          </FuzzyText>
        </div>
        <FuzzyText fontSize="clamp(1.6rem, 2.8vw, 2.5rem)" fontWeight={900} color="#000" baseIntensity={0.1} hoverIntensity={0.42} enableHover gradient={['#7c3aed', '#db2777', '#2563eb']}>
        ...
        {PILLARS.map(({ number, title, body }, i) => (
          ...
          >
            <SplitText
              text={number}
              tag="span"
              className="text-xs text-black/20 tracking-[0.2em] mb-8"
              delay={30}
              duration={0.6}
              ease="power2.out"
              splitType="chars"
            ...
            <SplitText
              text={title}
              tag="h3"
              className="text-2xl tracking-tight text-black mb-4"
              delay={22}
              duration={0.9}
              ease="power3.out"
              splitType="chars"
